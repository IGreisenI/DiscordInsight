...
namespace Dissonity
{
    ...
    public class DiscordBridge : MonoBehaviour
    {
        ...
        public void ReceiveGuildList(string stringData)
        {

            // Parse string
            GuildList data = JsonUtility.FromJson<GuildList>("{\"guilds\":" + stringData + "}");

            // Send data to subscriptions
            if (_GetGuildListDataEvent != null)
            {
                _GetGuildListDataEvent(data);
            }

            //¡ Clear delegates
            _GetGuildListDataEvent = null;
        }
